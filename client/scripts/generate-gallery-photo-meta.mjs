...
export const RAW_EXT = new Set([".nef", ".raf", ".dng", ".cr2", ".cr3", ".arw"]);
export { ORIGINALS_ROOT, OUT as PHOTO_META_OUT, REGIONS };
 ...
 */
export function exiftoolEnrichment(filePaths) {
 ...
 */
export async function metaForFile(filePath, toolRow) {
...
export async function metaForRegion(region) {
...
const META_OBJECT_RE = /export const GALLERY_PHOTO_META = (\{[\s\S]*?\});\s*\n/;

export function loadPhotoMeta() {
  if (!fs.existsSync(OUT)) return {};
  const prev = fs.readFileSync(OUT, "utf8");
  const match = prev.match(META_OBJECT_RE);
  return match ? JSON.parse(match[1]) : {};
}

export function writePhotoMeta(data) {
...
async function main() {
  ...
  let data = {};
  if (regionArg && fs.existsSync(OUT)) data = loadPhotoMeta();
