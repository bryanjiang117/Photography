...
import { CALIFORNIA_GALLERY_PHOTOS } from "../constants/data";
...
export default function CaliforniaGallery({ entrance = true, slide = true }) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-california-primary"
    >
      <motion.div
      ...
      >
        <div className="flex h-full w-max min-w-full flex-row items-center gap-3 px-4 py-3">
          {CALIFORNIA_GALLERY_PHOTOS.map((photo, i) => (
            <GalleryImage
              key={photo.name}
              region="california"
              entry={photo}
      ...
      >
        <button
          onClick={() => setShowCaliforniaGallery(false)}
